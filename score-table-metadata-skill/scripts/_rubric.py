...
def _evidence(text: str | None, max_chars: int = 90) -> str:
    """Compact preview of a description for the criteria's ``evidence`` field."""
    norm = _norm(text)
    if not norm:
        return ""
...
_CAVEAT_KEYS = ("deprecated", "legacy", "do not use", "null when", "null if", "overloaded",
                "multiple meanings", "reserved", "to be removed", "will be removed", "do not rely")
...
def _check_caveats(col: dict[str, Any]) -> dict[str, Any]:
    ...
    # Caveats only count when applicable; treat as non-applicable (max 0) otherwise
